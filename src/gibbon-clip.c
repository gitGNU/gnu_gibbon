...
static gboolean gibbon_clip_parse_win_game (const gchar *line,
                                            gchar **tokens,
                                            GSList **result);
static gboolean gibbon_clip_parse_win_match (const gchar *line,
                                             gchar **tokens,
                                             GSList **result);
                         ...
                         GSList **result)
{
        ...
        *result = gibbon_clip_alloc_int (*result,
        ...
        /* Non-Crawford.  */
        if (!gibbon_clip_extract_integer (tokens[50], &i64,
                                          0, G_MAXINT))
                goto bail_out_board;
        *result = gibbon_clip_alloc_int (*result,
                                         GIBBON_CLIP_TYPE_BOOLEAN,
...
static gboolean
gibbon_clip_parse_win_match (const gchar *line, gchar **tokens, GSList **result)
{
        gint64 match_length;
        gint64 s0, s1;
        gchar *ptr;
        ...
        if (!gibbon_clip_extract_integer (tokens[3], &match_length, 1, G_MAXINT))
                return FALSE;
        ...
        if (!ptr)
                return FALSE;
        *ptr++ = 0;
        if (!gibbon_clip_extract_integer (tokens[6], &s0, 0, G_MAXINT))
                return FALSE;
        gibbon_clip_chomp (ptr, '.');
                              ...
                              GSList **result)
{
        ...
        if (0 == g_strcmp0 ("invited", tokens[2])
            ...
            && !tokens[9]) {
                *result = gibbon_clip_alloc_int (*result,
                                                 GIBBON_CLIP_TYPE_UINT,
                                          GIBBON_CLIP_RESUME_CONFIRMATION);
                *result = gibbon_clip_alloc_string (*result,
                                                    GIBBON_CLIP_TYPE_NAME,
                                                    tokens[3]);
                return TRUE;
...
gibbon_clip_parse_you (const gchar *line, gchar **tokens, GSList **result)
{
        ...
        if (0 == g_strcmp0 ("win", tokens[1])
            && 0 == g_strcmp0 ("the", tokens[2])) {
                if (0 == g_strcmp0 ("game", tokens[3]))
                        return gibbon_clip_parse_win_game (line, tokens,
                                                           result);
                return gibbon_clip_parse_win_match (line, tokens, result);
        }
                                     ...
                                     GSList **result)
{
        ...
        *result = gibbon_clip_alloc_int (*result, GIBBON_CLIP_TYPE_UINT,
                                       GIBBON_CLIP_ERROR_NO_EMAIL_ADDRESS);
