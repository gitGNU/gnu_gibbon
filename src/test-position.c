...
static gboolean test_who_is_on_move (void);
static const char *side2string (GibbonPositionSide side);
...
test_who_is_on_move ()
{
        ...
        position->dice[0] = 0;
        position->dice[1] = 0;
        side = gibbon_position_on_move (position);
        if (GIBBON_POSITION_SIDE_NONE != side) {
                g_printerr ("Expected `none', got `%s' for dice %d/%d.\n",
                            side2string (side),
        ...
        position->dice[1] = -4;
        side = gibbon_position_on_move (position);
        if (GIBBON_POSITION_SIDE_BLACK != side) {
                g_printerr ("Expected `black', got `%s' for dice %d/%d.\n",
                            side2string (side),
                            position->dice[0], position->dice[1]);
                retval = FALSE;
        }
        ...
        position->dice[1] = -2;
        side = gibbon_position_on_move (position);
        if (GIBBON_POSITION_SIDE_WHITE != side) {
                g_printerr ("Expected `white', got `%s' for dice %d/%d.\n",
                            side2string (side),
                            position->dice[0], position->dice[1]);
                retval = FALSE;
        }
...
static const gchar *
side2string (GibbonPositionSide side)
{
